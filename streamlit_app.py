...
"""
===============================================================================
FlwrAI: Hoya Species Classifier - PERFECT VERSION
No white space, typewriter font, bigger text
===============================================================================
"""
...
# ============================================================================
# CSS - TYPEWRITER FONT + BIGGER TEXT + NO WHITE SPACE
# ============================================================================

st.markdown("""
    <style>
    /* Import typewriter fonts */
    @import url('https://fonts.googleapis.com/css2?family=Courier+Prime:wght@400;700&family=Special+Elite&family=IBM+Plex+Mono:wght@400;600;700&display=swap');
    
    /* Global font - Typewriter style */
    * {
...
def create_confidence_gauge(confidence):
    ...
    fig.update_layout(height=300, margin=dict(l=20, r=20, t=60, b=20), paper_bgcolor='rgba(0,0,0,0)', font={'color': '#1e293b', 'family': 'Courier Prime'})
    return fig
...
def create_probability_chart(species_probs, top_n=8):
    ...
    fig = go.Figure(go.Bar(
        ...
        textfont=dict(size=14, color='#1e293b', weight='bold', family='Courier Prime')
    ))
    
    fig.update_layout(
        title=dict(text="Top Candidates", font=dict(size=22, color='#1e293b', family='Courier Prime')),
...
def main():
    ...
    with tab2:
        ...
        with col1:
            st.markdown("**Dataset:** 64 specimens, 49 species")
            st.markdown("**Features:** 10 raw + 24 engineered")
        with col2:
            st.markdown("**Models:** RF, XGBoost, LightGBM, CatBoost")
    ...
    with tab4:
        st.markdown('<div class="warning-card">⚠️ <b>Research prototype</b> • Expert verification required</div>', unsafe_allow_html=True)
    
    st.markdown('</div>', unsafe_allow_html=True)
    
    # Main: Input LEFT | Output RIGHT - NO CONTAINERS
    ...
    with col_output:
        ...
        if 'prediction' in st.session_state:
            ...
            st.markdown(f'<div class="prediction-result"><p style="margin:0; color:#64748b;">PREDICTED SPECIES</p><p class="species-name">Hoya {predicted_species}</p><p class="confidence-text">{confidence*100:.1f}% Confidence</p></div>', unsafe_allow_html=True)
...
"""
            ...
            st.markdown('<div class="info-card" style="text-align:center; padding:2rem;"><h3>👈 Enter measurements</h3><p>Input data and click "IDENTIFY SPECIES"</p><p style="font-size:2rem; margin-top:1rem;">🌺 🔬</p></div>', unsafe_allow_html=True)
    
    st.markdown('<div style="text-align:center; color:#64748b; padding:1rem; font-size:1rem;">v2.0 | Research Only | 🌺 🌸</div>', unsafe_allow_html=True)
