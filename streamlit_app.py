...
st.markdown("""
    ...
    /* FIX: High-Contrast Tab Visibility */
...
@st.cache_data
def get_hoya_data():
    # Data represents known clades: Acanthostemma and Hoya-Complex
...
# --- APP LAYOUT ---
st.write("<h1 style='text-align: center; color: #2E4D32;'>🌿 Hoya Morpho-ID: AI-Powered Clade-Level Classifier</h1>", unsafe_allow_html=True)
st.write("<p style='text-align: center; color: #7A8B7C;'><i>Note: This system provides diagnostic predictions at the Clade level only.</i></p>", unsafe_allow_html=True)
...
with tab1:
    ...
    with col_l:
        ...
        st.write("Standard identification keys often create bottlenecks in conservation. This AI-powered application democratizes expert-level identification through a nature-based technical solution focused on major taxonomic clades.")
        
        st.write("### 🏷️ Clades Covered")
        st.markdown("""
        <span class="clade-tag">Acanthostemma</span>
        <span class="clade-tag">Hoya-Complex</span>
        """, unsafe_allow_html=True)
        st.write("<small><i>Classification is currently limited to these primary evolutionary groups.</i></small>", unsafe_allow_html=True)
    ...
    with col_r:
        ...
        fig_ref.update_layout(height=450, margin=dict(l=0,r=0,b=0,t=0), showlegend=False)
        st.plotly_chart(fig_ref, use_container_width=True)

# --- TAB 2: DIAGNOSTIC FACILITY ---
with tab2:
    ...
    with col_in:
        ...
        if st.button("Generate Diagnostic Report", use_container_width=True):
            ...
            st.session_state.test_log.append({"Time": datetime.datetime.now().strftime("%H:%M:%S"), "Sample": "Single", "P_L": p_l, "P_W": p_w, "C_L": c_l, "Clade Result": pred, "Confidence": f"{conf:.1%}"})
...
with tab3:
    ...
    if up:
        ...
        b_df['Clade Prediction'] = [model.classes_[i] for i in np.argmax(b_pr, axis=1)]; b_df['Confidence'] = np.max(b_pr, axis=1)
